...
public:
    ...
    friend class Seq;
private:
    void Adjust(double rate, const Matrix &weight_updates, const Vector &bias_updates);
    Matrix weights;
    Vector biases;
};
...
class Seq {
public:
    Seq(std::initializer_list<size_t> dimensions, std::vector<std::unique_ptr<ActivationFunction>> &&activations);
    Seq(std::vector<Layer> layers, std::vector<std::unique_ptr<ActivationFunction>> &&activations);
