...
void Layer::Adjust(double rate, const Matrix &weight_updates,
                   const Vector &bias_updates) {
  weights -= weight_updates * rate;
  biases -= bias_updates * rate;
}
Seq::Seq(std::initializer_list<size_t> dimensions,
...
Seq::Seq(std::vector<Layer> layers_t,
         std::vector<std::unique_ptr<ActivationFunction>> &&activations)
    : layers(std::move(layers_t)),
      activation_functions(std::move(activations)) {
  layer_count = activation_functions.size();
  gradient_w.resize(layer_count);
  gradient_b.resize(layer_count);
...
void Seq::Forward(Vector &input) {
  for (size_t i = 0; i < layer_count; ++i) {
    ...
    outputs_saved[i] =
        activation_functions[i]->Activate(layers[i].Activate(input));
    input = outputs_saved[i];
  }
}
void Seq::Backward(Vector &input) {
  for (int32_t i = static_cast<int>(layer_count) - 1; i >= 0; --i) {
    Vector activated_deriv =
        activation_functions[i]->Derivative(outputs_saved[i]) * input;
...
friend std::istream &operator>>(std::istream &input, Seq &sequence) {
  ...
  for (size_t i = 0; i < layer_count * 2; i++) {
    ...
    if (layer_name == "Layer") {
      ...
      activation_functions_t.push_back(
          ActivationFunctionFactory::Create(layer_name));
    }
  }
