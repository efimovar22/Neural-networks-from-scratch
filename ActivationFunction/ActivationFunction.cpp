...
[[nodiscard]] std::string Sigmoid::functionName() const { return "Sigmoid"; }
...
[[nodiscard]] Matrix Sigmoid::computeJacobian(const Vector &x) const {
  return ((-x.array()).exp() / pow(1.0 + (-x.array()).exp(), 2))
      .matrix()
      .asDiagonal();
}

[[nodiscard]] std::string ReLu::functionName() const { return "ReLu"; }
...
[[nodiscard]] std::string Softmax::functionName() const { return "Softmax"; }
