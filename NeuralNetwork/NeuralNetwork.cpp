...
NetworkModel::NetworkModel(Sequential seq,
                           std::unique_ptr<LossFunction> loss_function,
                           double learning_rate)
    : learning_rate(learning_rate), sequential(std::move(seq)),
...
void NetworkModel::Train(const std::string &path_to_images,
                         const std::string &path_to_labels, size_t batch_size,
                         size_t epochs) {
...
NetworkModel::Predict(const std::string &path_to_images,
                      const std::string &path_to_labels, size_t batch_size) {
  DataLoader data_loader(path_to_images, path_to_labels, batch_size);
  data_loader.Restart();
