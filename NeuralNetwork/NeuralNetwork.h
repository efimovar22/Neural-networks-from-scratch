...
class NetworkModel {
public:
  NetworkModel(Seq seq, std::unique_ptr<LossFunction> loss_function,
               double learning_rate);
  void Train(const std::string &path_to_images,
             const std::string &path_to_labels, size_t batch_size,
             size_t epoch);
  std::pair<size_t, size_t> Predict(const std::string &path_to_images,
